...
def openInterface():
    ...
    lbl_qf = tk.Label(window.frame, text="F:").grid(row=9, column=1, sticky="W", pady=3, padx=5)
    entry_qf = tk.Entry(window.frame, width=30)
    entry_qf.grid(row=9, column=2, pady=3, padx=5)

    lbl_alphabet = tk.Label(window.frame, text="V:").grid(row=10, column=1, sticky="W", pady=3, padx=5)
    entry_word = tk.Entry(window.frame, width=30)
    entry_word.grid(row=10, column=2, pady=3, padx=5)

    lbl_word = tk.Label(window.frame, text="Palavra a ser lida:").grid(row=10, column=1, sticky="W", pady=3, padx=5)
    entry_word = tk.Entry(window.frame, width=30)
    entry_word.grid(row=10, column=2, pady=3, padx=5)

    # Generate an automaton
    btn_create_automaton = tk.Button(window.frame, text="Criar automato").grid(row=12, column=1, pady=3, padx=5)
    lbl_automaton = tk.Label(text="")
    lbl_automaton.grid(row=12, column=3, pady=3, padx=5)

    # Create the run button
    btn_run = tk.Button(window.frame, text="Testar", command=window.run_afd).grid(row=14, column=1, pady=3, padx=5)
